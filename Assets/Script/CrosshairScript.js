...
function Update () {
	...
	if (targetScript.IsAimCharged()) {
		...
		this.transform.position = targetScript.GetCarryingPosition() + (target.forward * chargedDistance * Mathf.Cos(aimAngleRads)) + (target.up * chargedDistance * Mathf.Sin(aimAngleRads));
